    ...
    if use_interp:
        ip_funcs = [
            interp1d(
                ...
                kind=interp_kind,
                bounds_error=False,
                fill_value=u[R - 1, j, -1],
            )
            for j in range(J)
        ]

        H_td_func = [H0] + [
            [H_ops[j], lambda t, args, j=j: ip_funcs[j](t)] for j in range(J)
        ]
    else:
...
def cy_grape_unitary(
    ...
    for r in range(R - 1):
        ...
        if use_interp:
            ...
            U_list = [
                (-1j * _H_t(times[idx]) * dt).expm().data
            ...
            U_list = [
                (-1j * _H_idx(idx) * dt).expm().data for idx in range(M - 1)
            ]
