...
_System = collections.namedtuple(
    "_System", ["system", "controls", "initial", "target", "kwargs"]
)
...
_qft_controls = [
    0.5 * qutip.tensor(_sx, _si),
    0.5 * qutip.tensor(_sy, _si),
    0.5 * qutip.tensor(_si, _sx),
    0.5 * qutip.tensor(_si, _sy),
]
_qft_kwargs = {
    "num_tslots": 10,
    "evo_time": 10,
...
_ising_system = (
    0.9 * qutip.tensor(_sx, _si)
    + 0.7 * qutip.tensor(_si, _sx)
    + 0.8 * qutip.tensor(_sz, _si)
    + 0.9 * qutip.tensor(_si, _sz)
)
_ising_kwargs = {
    "num_tslots": 10,
    "evo_time": 18,
...
_l_adc_system = 0.1 * (
    2 * qutip.tensor(_sm, _sp.dag())
    - qutip.tensor(_project_0, _si)
    - qutip.tensor(_si, _project_0.dag())
)
_l_adc_controls = [
    1j * (qutip.tensor(_si, _sz) - qutip.tensor(_sz, _si)),
...
_sympl_system = qutip.qdiags(
    [[1, 1, 1, 1], [_g1, _g2], [_g1, _g2]], [0, 2, -2]
)
...
def _optimize_pulse(system):
    ...
    error = " ".join(
        [
            "Infidelity: {:7.4e}".format(result.fid_err),
            "reason:",
...
def _load_pulse_generator(path, configuration, dynamics):
    pulse_generator = qutip_qtrl.pulsegen.create_pulse_gen(
        pulse_type=configuration.pulse_type, dyn=dynamics
    )
    qutip_qtrl.loadparams.load_parameters(str(path), pulsegen=pulse_generator)
...
def _load_optimizer(
    ...
    constructor = getattr(qutip_qtrl.optimizer, known.get(method, "Optimizer"))
...
class TestFileIO:
    def test_load_parameters_from_file(self):
        ...
        np.testing.assert_allclose(
            result.final_amps, target.final_amps, atol=1e-5
        )
    ...
    def test_dumping_to_files(self):
        ...
        assert (
            len(os.listdir(result.optimizer.dump.dump_dir))
            == N_OPTIMDUMP_FILES
        )
        assert (
            len(os.listdir(result.optimizer.dynamics.dump.dump_dir))
            == N_DYNDUMP_FILES
        )

        # Dump all to specific file stream.
        for dump, type_ in [
            (result.optimizer.dump, "optimizer"),
            (result.optimizer.dynamics.dump, "dynamics"),
        ]:
            with tempfile.NamedTemporaryFile() as file:
                dump.writeout(file)
                assert os.stat(file.name).st_size > 0, " ".join(
                    ["Empty", type_, "file."]
                )
...
class TestTimeDependence:
    ...
    def test_drift(self):
        ...
        num_tslots = 20
        system = _merge_kwargs(
            hadamard, {"num_tslots": num_tslots, "evo_time": 10}
        )
        ...
        np.testing.assert_allclose(
            result_fixed.final_amps, result_flat.final_amps, rtol=1e-9
        )
        assert np.any(
            (result_flat.final_amps - result_step.final_amps) > 1e-3
    ...
    def test_controls_all_time_slots_equal_to_no_time_dependence(self):
        ...
        system = _merge_kwargs(
            hadamard,
            {"num_tslots": num_tslots, "evo_time": 10, "fid_err_targ": 1e-10},
        )
        ...
        np.testing.assert_allclose(
            result_single.final_amps, result_vary.final_amps, atol=1e-9
        )
